...
namespace Beta.Data.MyCatSqlClient
{
  ...
  internal class CharSetMap
  {
    ...
    internal static void InitCollections(MyCatSqlConnection connection)
    {
      _defaultCollations = new Dictionary<string, string>();
      _maxLengths = new Dictionary<string, int>();

      MyCatSqlCommand cmd = new MyCatSqlCommand("SHOW CHARSET", connection);
    ...
    internal static string GetDefaultCollation(string charset, MyCatSqlConnection connection)
    {
      lock (LockObject)
      {
    ...
    internal static int GetMaxLength(string charset, MyCatSqlConnection connection)
    {
      lock (LockObject)
      {
        if (_maxLengths == null)
