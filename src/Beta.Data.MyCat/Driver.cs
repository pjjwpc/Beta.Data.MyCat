...
namespace Beta.Data.MyCatSqlClient
{
    ...
    internal partial class Driver : IDisposable
    {
        ...
        protected MyCatSqlConnectionStringBuilder ConnectionString;
        ...
        public Driver(MyCatSqlConnectionStringBuilder settings)
        {
        ...
        public MyCatSqlConnectionStringBuilder Settings
        {
        ...
        public MyCatSqlPool Pool { get; set; }
        ...
        public static Driver Create(MyCatSqlConnectionStringBuilder settings)
        {
            Driver d = null;

            try
            {
                if (MyCatSqlTrace.QueryAnalysisEnabled || settings.Logging || settings.UseUsageAdvisor)
        ...
        public virtual void Configure(MyCatSqlConnection connection)
        {
      ...
      if (licenseType == null || licenseType.Length == 0 || 
        ...
        throw new MySqlException( "This client library licensed only for use with commercially-licensed MySQL servers." );
            ...
            // want results in
            MyCatSqlCommand charSetCmd = new MyCatSqlCommand($"SET character_set_results={charSet}",
              connection)
            ...
            {
                MyCatSqlCommand setNamesCmd = new MyCatSqlCommand("SET NAMES " + charSet, connection);
        ...
        /// <returns></returns>
        private Dictionary<string, string> LoadServerProperties(MyCatSqlConnection connection)
        {
            // load server properties
            Dictionary<string, string> hash = new Dictionary<string, string>();
            MyCatSqlCommand cmd = new MyCatSqlCommand("SHOW VARIABLES", connection);
            try
            {
                using (MyCatSqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
            ...
            {
                MyCatSqlTrace.LogError(ThreadID, ex.Message);
                throw;
            }
        }

        private int GetTimeZoneOffset(MyCatSqlConnection con)
        {
            MyCatSqlCommand cmd = con.CreateCommand();
            switch (ConnectionString.DbType)
            {
                case "sqlserver":
                    cmd.CommandText = "SELECT Datediff(hh,getutcdate(),Getdate()) ";
                    int? timediff = cmd.ExecuteScalar() as int?;
                    return timediff.HasValue ? timediff.Value : 0;
                case "pgsql":
                    return 0;
                case "oracle":
                    return 0;
                case "mysql":
                default:
                    cmd.CommandText = "SELECT TIMEDIFF(NOW(), UTC_TIMESTAMP())";
        ...
        /// </summary>
        private void LoadCharacterSets(MyCatSqlConnection connection)
        {
            MyCatSqlCommand cmd = new MyCatSqlCommand("SHOW COLLATION", connection);
        ...
        public virtual List<MyCatSqlError> ReportWarnings(MyCatSqlConnection connection)
        {
            List<MyCatSqlError> warnings = new List<MyCatSqlError>();

            MyCatSqlCommand cmd = new MyCatSqlCommand("SHOW WARNINGS", connection) { InternallyCreated = true };
            using (MyCatSqlDataReader reader = cmd.ExecuteReader())
            {
                while (reader.Read())
                {
                    warnings.Add(new MyCatSqlError(reader.GetString(0),
        ...
        public virtual void SendQuery(MyCatSqlPacket p)
        {
            handler.SendQuery(p);
            firstResult = true;
        }
        ...
        {
            MyCatSqlPacket p = new MyCatSqlPacket(Encoding);
            p.WriteString(sql);
            SendQuery(p);
            NextResult(0, false);
        }
        ...
        public virtual void ExecuteStatement(MyCatSqlPacket packetToExecute)
        {
        ...
        public virtual void CloseQuery(MyCatSqlConnection connection, int statementId)
        {
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            try
            {
                ...
                if (ConnectionString.Pooling)
                    MyCatSqlPoolManager.RemoveConnection(this);
            }
            catch (Exception ex)
            {
                if (disposing)
                {
                    ...
                    if (mysqlEx == null)
                        MyCatSqlTrace.LogError(0, ex.GetBaseException().Message);
                    else
                        MyCatSqlTrace.LogError(mysqlEx.Number, ex.GetBaseException().Message);
                }
            }
            finally
            {
    ...
    internal interface IDriver
    {
        ...
        void Configure();
        void Open();
        void SendQuery(MyCatSqlPacket packet);
        ...
        void ExecuteStatement(MyCatSqlPacket packet);
