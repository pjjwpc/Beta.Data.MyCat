...
namespace Beta.Data.Failover
{
  ...
  internal static class FailoverManager
  {
    ...
    /// Attempts to establish a connection to a host specified from the list.
    /// </summary>
    /// <param name="connection">MyCatSqlConnection object where the new driver will be assigned</param>
    /// <param name="originalConnectionString">The original connection string set by the user.</param>
    /// <param name="connectionString">An out parameter that stores the updated connection string.</param>
    /// <param name="mySqlPoolManager">A <see cref="MyCatSqlPoolManager"> in case this is a pooling scenario."/></param>
    internal static void AttemptConnection(MyCatSqlConnection connection, string originalConnectionString, out string connectionString, bool mySqlPoolManager = false)
    {
      if (mySqlPoolManager)
        if (MyCatSqlPoolManager.Hosts == null)
        {
          MyCatSqlPoolManager.Hosts = FailoverGroup.Hosts;
          MyCatSqlPoolManager.DemotedHosts = new ConcurrentQueue<FailoverServer>();
        }
        else
          FailoverGroup.Hosts = MyCatSqlPoolManager.Hosts;
      ...
      {
        // Attempt to connect to each host by retrieving the next host based on the failover method being used
        MyCatSqlConnectionStringBuilder msb;
        ...
        msb = new MyCatSqlConnectionStringBuilder(connectionString);

        if ((FailoverGroup.Hosts.Count == 1 && !mySqlPoolManager) ||
          (mySqlPoolManager && MyCatSqlPoolManager.Hosts.Count == 1 && MyCatSqlPoolManager.DemotedHosts.IsEmpty))
          return;
        ...
        {
          tmpHost.DemotedTime = DateTime.Now;
          MyCatSqlPoolManager.Hosts.Remove(tmpHost);
          MyCatSqlPoolManager.DemotedHosts.Enqueue(tmpHost);

          if (MyCatSqlPoolManager.DemotedServersTimer == null)
            MyCatSqlPoolManager.DemotedServersTimer = new Timer(new TimerCallback(MyCatSqlPoolManager.ReleaseDemotedHosts),
              null, MyCatSqlPoolManager.DEMOTED_TIMEOUT, Timeout.Infinite);
        }
      } while (!currentHost.Equals(initialHost));
