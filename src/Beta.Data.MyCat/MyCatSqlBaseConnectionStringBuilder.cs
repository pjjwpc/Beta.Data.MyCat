...
using static Beta.Data.Common.MyCatSqlConnectionStringOption;
...
namespace Beta.Data.MyCatSqlClient
{
  ...
  /// </summary>
  public abstract class MyCatSqlBaseConnectionStringBuilder : DbConnectionStringBuilder
  {
    /// <summary>
    /// Readonly field containing a collection of protocol shared connection options.
    /// </summary>
    internal static readonly MyCatSqlConnectionStringOptionCollection Options = new MyCatSqlConnectionStringOptionCollection();

    static MyCatSqlBaseConnectionStringBuilder()
    {
      // Server options.
      Options.Add(new MyCatSqlConnectionStringOption("server", "host,data source,datasource,address,addr,network address", typeof(string), "" /*"localhost"*/, false));
      Options.Add(new MyCatSqlConnectionStringOption("database", "initial catalog", typeof(string), string.Empty, false));
      Options.Add(new MyCatSqlConnectionStringOption("protocol", "connection protocol,connectionprotocol", typeof(MyCatSqlConnectionProtocol), MyCatSqlConnectionProtocol.Sockets, false,
        (SetterDelegate)((msb, sender, value) =>
        {
#if !NET452
         MyCatSqlConnectionProtocol enumValue;
         if (Enum.TryParse<MyCatSqlConnectionProtocol>(value.ToString(), true, out enumValue))
         {
           if (enumValue == MyCatSqlConnectionProtocol.Memory || enumValue == MyCatSqlConnectionProtocol.Pipe)
      ...
      Options.Add(new MyCatSqlConnectionStringOption("port", null, typeof(uint), (uint)3306, false));
      Options.Add(new MyCatSqlConnectionStringOption("dns-srv", "dnssrv", typeof(bool), false, false));

      // Authentication options.
      Options.Add(new MyCatSqlConnectionStringOption("user id", "uid,username,user name,user,userid", typeof(string), "", false));
      Options.Add(new MyCatSqlConnectionStringOption("password", "pwd", typeof(string), "", false));
      Options.Add(new MyCatSqlConnectionStringOption("certificatefile", "certificate file", typeof(string), null, false));
      Options.Add(new MyCatSqlConnectionStringOption("certificatepassword", "certificate password,ssl-ca-pwd", typeof(string), null, false));
      Options.Add(new MyCatSqlConnectionStringOption("certificatestorelocation", "certificate store location", typeof(MySqlCertificateStoreLocation), MySqlCertificateStoreLocation.None, false));
      Options.Add(new MyCatSqlConnectionStringOption("certificatethumbprint", "certificate thumb print", typeof(string), null, false));
      Options.Add(new MyCatSqlConnectionStringOption("sslmode", "ssl mode,ssl-mode", typeof(MySqlSslMode), MySqlSslMode.Preferred, false,
        (SetterDelegate)((msb, sender, value) =>
        {
      ...
      Options.Add(new MyCatSqlConnectionStringOption("sslca", "ssl-ca", typeof(string), null, false,
        (SetterDelegate)((msb, sender, value) => { msb.SslCa = value as string; }),
        (GetterDelegate)((msb, sender) => { return msb.SslCa; })));
      Options.Add(new MyCatSqlConnectionStringOption("sslkey", "ssl-key", typeof(string), null, false));
      Options.Add(new MyCatSqlConnectionStringOption("sslcert", "ssl-cert", typeof(string), null, false));
      Options.Add(new MyCatSqlConnectionStringOption("tlsversion", "tls-version,tls version", typeof(string), null, false,
        (SetterDelegate)((msb, sender, value) =>
        {
      ...
      Options.Add(new MyCatSqlConnectionStringOption("sshhostname", "ssh host name,ssh-host-name", typeof(string), "", false));
      Options.Add(new MyCatSqlConnectionStringOption("sshport", "ssh port,ssh-port", typeof(uint), (uint)22, false));
      Options.Add(new MyCatSqlConnectionStringOption("sshusername", "ssh user name,ssh-user-name", typeof(string), "", false));
      Options.Add(new MyCatSqlConnectionStringOption("sshpassword", "ssh password,ssh-password", typeof(string), "", false));
      Options.Add(new MyCatSqlConnectionStringOption("sshkeyfile", "ssh key file,ssh-key-file", typeof(string), "", false));
      Options.Add(new MyCatSqlConnectionStringOption("sshpassphrase", "ssh pass phrase,ssh-pass-phrase", typeof(string), "", false));

      // Other properties.
      Options.Add(new MyCatSqlConnectionStringOption("keepalive", "keep alive", typeof(uint), (uint)0, false));

      // Language and charset options.
      Options.Add(new MyCatSqlConnectionStringOption("characterset", "character set,charset", typeof(string), "", false));
    }
    ...
    [RefreshProperties(RefreshProperties.All)]
    public MyCatSqlConnectionProtocol ConnectionProtocol
    {
      get { return (MyCatSqlConnectionProtocol)values["protocol"]; }
    ...
    internal void AnalyzeConnectionString(string connectionString, bool isXProtocol, bool isDefaultPort = true)
    {
      ...
      foreach (string query in queries)
      {
        ...
        MyCatSqlConnectionStringOption option = Options.Options.Where(o => o.Keyword == keyword || (o.Synonyms != null && o.Synonyms.Contains(keyword))).FirstOrDefault();
    ...
    internal abstract MyCatSqlConnectionStringOption GetOption(string key);
