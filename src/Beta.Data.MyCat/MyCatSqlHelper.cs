...
namespace Beta.Data.MyCatSqlClient
{
  ...
  /// </summary>
  public sealed partial class MyCatSqlHelper
  {
    enum CharClass : byte
    {
      None,
    ...
    /// <summary>
    /// Executes a single command against a MySQL database.  The <see cref="MyCatSqlConnection"/> is assumed to be
    /// open when the method is called and remains open after the method completes.
    /// </summary>
    /// <param name="connection">The <see cref="MyCatSqlConnection"/> object to use</param>
    ...
    public static int ExecuteNonQuery(MyCatSqlConnection connection, string commandText, params MySqlParameter[] commandParameters)
    {
      //create a command and prepare it for execution
      MyCatSqlCommand cmd = new MyCatSqlCommand();
    ...
    /// </summary>
    /// <param name="connectionString"><see cref="MyCatSqlConnection.ConnectionString"/> to use.</param>
    /// <param name="commandText">The SQL command to be executed.</param>
    /// <param name="parms">An rray of <see cref="MySqlParameter"/> objects to use with the command.</param>
    /// <returns>The number of affected records.</returns>
    /// <remarks>A new <see cref="MyCatSqlConnection"/> is created using the <see cref="MyCatSqlConnection.ConnectionString"/> given.</remarks>
    public static int ExecuteNonQuery(string connectionString, string commandText, params MySqlParameter[] parms)
    {
      //create & open a SqlConnection, and dispose of it after we are done.
      using (MyCatSqlConnection cn = new MyCatSqlConnection(connectionString))
      {
        cn.Open();
    ...
    /// </summary>
    /// <param name="connection"><see cref="MyCatSqlConnection"/> object to use for the command</param>
    ...
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    private static MyCatSqlDataReader ExecuteReader(MyCatSqlConnection connection, MySqlTransaction transaction, string commandText, MySqlParameter[] commandParameters, bool externalConn)
    {
      //create a command and prepare it for execution
      MyCatSqlCommand cmd = new MyCatSqlCommand();
      cmd.Connection = connection;
    ...
    /// <param name="commandText">Command text to use</param>
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    public static MyCatSqlDataReader ExecuteReader(string connectionString, string commandText)
    {
    ...
    /// <param name="commandText">Command text to use</param>
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    public static MyCatSqlDataReader ExecuteReader(MyCatSqlConnection connection, string commandText)
    {
      //pass through the call providing null for the set of SqlParameters
    ...
    /// <param name="commandParameters">Array of <see cref="MySqlParameter"/> objects to use with the command</param>
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    public static MyCatSqlDataReader ExecuteReader(string connectionString, string commandText, params MySqlParameter[] commandParameters)
    {
      //create & open a SqlConnection
      MyCatSqlConnection cn = new MyCatSqlConnection(connectionString);
      cn.Open();
    ...
    /// <param name="commandParameters">Array of <see cref="MySqlParameter"/> objects to use with the command</param>
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    public static MyCatSqlDataReader ExecuteReader(MyCatSqlConnection connection, string commandText, params MySqlParameter[] commandParameters)
    {
      //call the private overload that takes an internally owned connection in place of the connection string
    ...
    /// Execute a single command against a MySQL database.
    /// </summary>
    /// <param name="connection"><see cref="MyCatSqlConnection"/> object to use</param>
    /// <param name="commandText">Command text to use for the command</param>
    /// <returns>The first column of the first row in the result set, or a null reference if the result set is empty.</returns>
    public static object ExecuteScalar(MyCatSqlConnection connection, string commandText)
    {
      //pass through the call providing null for the set of MySqlParameters
    ...
    /// <param name="commandParameters">Parameters to use for the command</param>
    /// <returns>The first column of the first row in the result set, or a null reference if the result set is empty.</returns>
    public static object ExecuteScalar(MyCatSqlConnection connection, string commandText, params MySqlParameter[] commandParameters)
    {
      //create a command and prepare it for execution
      MyCatSqlCommand cmd = new MyCatSqlCommand();
      cmd.Connection = connection;
    ...
    public static Task<int> ExecuteNonQueryAsync(MyCatSqlConnection connection, string commandText, params MySqlParameter[] commandParameters)
    {
      return ExecuteNonQueryAsync(connection, commandText, CancellationToken.None, commandParameters);
    }

    public static Task<int> ExecuteNonQueryAsync(MyCatSqlConnection connection, string commandText, CancellationToken cancellationToken, params MySqlParameter[] commandParameters)
    {
    ...
    /// <param name="ExternalConn">True if the connection should be preserved, false if not</param>
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    private static Task<MyCatSqlDataReader> ExecuteReaderAsync(MyCatSqlConnection connection, MySqlTransaction transaction, string commandText, MySqlParameter[] commandParameters, bool ExternalConn)
    {
      return ExecuteReaderAsync(connection, transaction, commandText, commandParameters, ExternalConn, CancellationToken.None);
    }

    private static Task<MyCatSqlDataReader> ExecuteReaderAsync(MyCatSqlConnection connection, MySqlTransaction transaction, string commandText, MySqlParameter[] commandParameters, bool ExternalConn, CancellationToken cancellationToken)
    {
      var result = new TaskCompletionSource<MyCatSqlDataReader>();
      if (cancellationToken == CancellationToken.None || !cancellationToken.IsCancellationRequested)
      {
        try
        {
    ...
    /// <param name="commandText">Command text to use</param>
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    public static Task<MyCatSqlDataReader> ExecuteReaderAsync(string connectionString, string commandText)
    {
      return ExecuteReaderAsync(connectionString, commandText, CancellationToken.None, (MySqlParameter[])null);
    }

    public static Task<MyCatSqlDataReader> ExecuteReaderAsync(string connectionString, string commandText, CancellationToken cancellationToken)
    {
    ...
    /// <param name="commandText">Command text to use</param>
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    public static Task<MyCatSqlDataReader> ExecuteReaderAsync(MyCatSqlConnection connection, string commandText)
    {
      return ExecuteReaderAsync(connection, null, commandText, (MySqlParameter[])null, true, CancellationToken.None);
    }

    public static Task<MyCatSqlDataReader> ExecuteReaderAsync(MyCatSqlConnection connection, string commandText, CancellationToken cancellationToken)
    {
    ...
    /// <param name="commandParameters">Array of <see cref="MySqlParameter"/> objects to use with the command</param>
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    public static Task<MyCatSqlDataReader> ExecuteReaderAsync(string connectionString, string commandText, params MySqlParameter[] commandParameters)
    {
      return ExecuteReaderAsync(connectionString, commandText, CancellationToken.None, commandParameters);
    }

    public static Task<MyCatSqlDataReader> ExecuteReaderAsync(string connectionString, string commandText, CancellationToken cancellationToken, params MySqlParameter[] commandParameters)
    {
      var result = new TaskCompletionSource<MyCatSqlDataReader>();
      if (cancellationToken == CancellationToken.None || !cancellationToken.IsCancellationRequested)
      {
    ...
    /// <param name="commandParameters">Array of <see cref="MySqlParameter"/> objects to use with the command</param>
    /// <returns><see cref="MyCatSqlDataReader"/> object ready to read the results of the command</returns>
    public static Task<MyCatSqlDataReader> ExecuteReaderAsync(MyCatSqlConnection connection, string commandText, params MySqlParameter[] commandParameters)
    {
      return ExecuteReaderAsync(connection, null, commandText, commandParameters, true, CancellationToken.None);
    }

    public static Task<MyCatSqlDataReader> ExecuteReaderAsync(MyCatSqlConnection connection, string commandText, CancellationToken cancellationToken, params MySqlParameter[] commandParameters)
    {
    ...
    /// <param name="commandText">Command text to use for the command</param>
    /// <returns>The first column of the first row in the result set, or a null reference if the result set is empty.</returns>
    public static Task<object> ExecuteScalarAsync(MyCatSqlConnection connection, string commandText)
    {
      return ExecuteScalarAsync(connection, commandText, CancellationToken.None, (MySqlParameter[])null);
    }

    public static Task<object> ExecuteScalarAsync(MyCatSqlConnection connection, string commandText, CancellationToken cancellationToken)
    {
    ...
    /// <param name="commandParameters">Parameters to use for the command</param>
    /// <returns>The first column of the first row in the result set, or a null reference if the result set is empty.</returns>
    public static Task<object> ExecuteScalarAsync(MyCatSqlConnection connection, string commandText, params MySqlParameter[] commandParameters)
    {
      return ExecuteScalarAsync(connection, commandText, CancellationToken.None, commandParameters);
    }

    public static Task<object> ExecuteScalarAsync(MyCatSqlConnection connection, string commandText, CancellationToken cancellationToken, params MySqlParameter[] commandParameters)
    {
      var result = new TaskCompletionSource<object>();
      if (cancellationToken == CancellationToken.None || !cancellationToken.IsCancellationRequested)
      {
