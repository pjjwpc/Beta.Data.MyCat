...
namespace Beta.Data.MyCatSqlClient
{
  ...
  /// </summary>
  internal sealed class MyCatSqlPool
  {
    ...
    public MyCatSqlPool(MyCatSqlConnectionStringBuilder settings)
    {
    ...
    public MyCatSqlConnectionStringBuilder Settings { get; set; }
    ...
    public void RemoveConnection(Driver driver)
    {
      ...
      if (BeingCleared && NumConnections == 0)
        MyCatSqlPoolManager.RemoveClearedPool(this);
    }
    ...
    private Driver TryToGetDriver()
    {
      ...
      {
        MyCatSqlTrace.LogError(-1, ex.Message);
        Interlocked.Increment(ref _available);
        throw;
      }
    }
      ...
      lock ((_idlePool as ICollection).SyncRoot)
      {
        ...
        while (_idlePool.Count > _minSize)
        {
          ...
          DateTime expirationTime = d.IdleSince.Add(
            new TimeSpan(0, 0, MyCatSqlPoolManager.maxConnectionIdleTime));
