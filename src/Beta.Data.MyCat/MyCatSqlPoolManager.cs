...
namespace Beta.Data.MyCatSqlClient
{
  ...
  /// </summary>
  internal class MyCatSqlPoolManager
  {
    private static readonly Dictionary<string, MyCatSqlPool> Pools = new Dictionary<string, MyCatSqlPool>();
    private static readonly List<MyCatSqlPool> ClearingPools = new List<MyCatSqlPool>();
    ...
    private static string GetKey(MyCatSqlConnectionStringBuilder settings)
    {
      string key = "";
      lock (settings)
      {
    ...
    public static MyCatSqlPool GetPool(MyCatSqlConnectionStringBuilder settings)
    {
      ...
      lock (Pools)
      {
        ...
        if (pool == null)
        {
          pool = new MyCatSqlPool(settings);
          Pools.Add(text, pool);
        }
        else
    ...
    public static void ClearPool(MyCatSqlConnectionStringBuilder settings)
    {
    ...
    private static void ClearPoolByText(string key)
    {
      lock (Pools)
      {
        ...
        MyCatSqlPool pool = (Pools[key] as MyCatSqlPool);
    ...
    public static void RemoveClearedPool(MyCatSqlPool pool)
    {
    ...
    public static void CleanIdleConnections(object obj)
    {
      ...
      lock (Pools)
      {
        foreach (MyCatSqlPool pool in Pools.Keys.Select(key => Pools[key]))
        {
