...
using Beta.Data.MyCatSqlClient.Authentication;
...
namespace Beta.Data.MyCatSqlClient
{
  ...
  internal class NativeDriver : IDriver
  {
    ...
    protected MyCatSqlStream stream;
    protected Stream baseStream;
    private BitArray nullMap;
    private MyCatSqlPacket packet;
    ...
    public MyCatSqlPacket Packet
    {
      get { return packet; }
    }

    internal MyCatSqlConnectionStringBuilder Settings
    {
    ...
    internal void SendPacket(MyCatSqlPacket p)
    {
      stream.SendPacket(p);
    }
    ...
    internal MyCatSqlPacket ReadPacket()
    {
    ...
    public void Open()
    {
      ...
      stream = new MyCatSqlStream(baseStream, Encoding, false);
      ...
      if ((connectionFlags & ClientFlags.COMPRESS) != 0)
        stream = new MyCatSqlStream(baseStream, Encoding, true);
    ...
    /// </summary>
    public void SendQuery(MyCatSqlPacket queryPacket)
    {
      warnings = 0;
    ...
    public void Close(bool isOpen)
    {
      try
      {
        if (isOpen)
        {
          ...
          {
            MyCatSqlTrace.LogError(ThreadId, ex.ToString());
    ...
    private void ExecutePacket(MyCatSqlPacket packetToExecute)
    {
      try
      {
        warnings = 0;
    ...
    public void ExecuteStatement(MyCatSqlPacket packetToExecute)
    {
      warnings = 0;
