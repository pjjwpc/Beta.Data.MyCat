...
namespace Beta.Data.MyCatSqlClient.Replication
{
  /// <summary>
  ...
  internal static class ReplicationManager
  {
    ...
    internal static ReplicationServerGroup AddGroup(string name, string groupType, int retryTime)
    {
      if (string.IsNullOrEmpty(groupType))
        groupType = "Beta.Data.MyCatSqlClient.Replication.ReplicationRoundRobinServerGroup";
    ...
    internal static void GetNewConnection(string groupName, bool master, MyCatSqlConnection connection)
    {
      do
      {
        lock (thisLock)
        {
          ...
          try
          {
            ...
            { 
              MyCatSqlConnectionStringBuilder msb = new MyCatSqlConnectionStringBuilder(server.ConnectionString);
            ...
            if (isNewServer)
            {
              Driver driver = Driver.Create(new MyCatSqlConnectionStringBuilder(server.ConnectionString));
          ...
          catch (MySqlException ex)
          {
            ...
            MyCatSqlTrace.LogError(ex.Number, ex.ToString());
            if (ex.Number == 1042)
            {
