...
namespace Beta.Data.MyCatSqlClient.Replication
{
  ...
  public abstract class ReplicationServerGroup
  {
    ...
    /// Null if no available server is found.
    /// </returns>
    internal protected virtual ReplicationServer GetServer(bool isMaster, MyCatSqlConnectionStringBuilder settings)
    {
    ...
    internal protected virtual void HandleFailover(ReplicationServer server)
    {
      ...
      worker.DoWork += delegate(object sender, DoWorkEventArgs e)
      {
        ...
        System.Timers.ElapsedEventHandler elapsedEvent = delegate(object sender1, System.Timers.ElapsedEventArgs e1)
        {
          ...
          {
            isRunning = true;
            using (MyCatSqlConnection connectionFailed = new MyCatSqlConnection(server.ConnectionString))
            {
