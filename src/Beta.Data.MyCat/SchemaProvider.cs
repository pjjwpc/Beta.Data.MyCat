...
namespace Beta.Data.MyCatSqlClient
{
  ...
  {
    protected MyCatSqlConnection connection;
    public static string MetaCollection = "MetaDataCollections";

    public SchemaProvider(MyCatSqlConnection connectionToUse)
    {
                    ...
                    string tableName, string columnRestriction)
    {
      string sql = String.Format("SHOW FULL COLUMNS FROM `{0}`.`{1}`",
                     schema, tableName);
      MyCatSqlCommand cmd = new MyCatSqlCommand(sql, connection);
    ...
    public virtual MySqlSchemaCollection GetIndexes(string[] restrictions)
    {
      ...
      foreach (MySqlSchemaRow table in tables.Rows)
      {
        string sql = String.Format("SHOW INDEX FROM `{0}`.`{1}`",
          MyCatSqlHelper.DoubleQuoteString((string)table["TABLE_SCHEMA"]),
          MyCatSqlHelper.DoubleQuoteString((string)table["TABLE_NAME"]));
    ...
    public virtual MySqlSchemaCollection GetProcedures(string[] restrictions)
    {
      ...
      if (connection.driver.Version.isAtLeast(8, 0, 1))
      {
        ...
        MyCatSqlCommand cmd = new MyCatSqlCommand(sql.ToString(), connection);
        using (MyCatSqlDataReader reader = cmd.ExecuteReader())
        {
          while (reader.Read())
          {
    ...
    internal static MySqlSchemaCollection GetReservedWords()
    {
      ...
      string resourceName = "Beta.Data.Properties.ReservedWords.txt";
    ...
    private static string GetString(MyCatSqlDataReader reader, int index)
    {
    ...
    protected MySqlSchemaCollection QueryCollection(string name, string sql)
    {
      ...
      MyCatSqlCommand cmd = new MyCatSqlCommand(sql, connection);
      MyCatSqlDataReader reader = cmd.ExecuteReader();
