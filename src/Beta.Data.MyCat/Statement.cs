...
namespace Beta.Data.MyCatSqlClient
{
  ...
  {
    protected  MyCatSqlCommand command;
    private readonly List<MyCatSqlPacket> _buffers;
    protected string commandText;

    private Statement(MyCatSqlCommand cmd)
    {
      command = cmd;
      _buffers = new List<MyCatSqlPacket>();
    }

    protected Statement(MyCatSqlCommand cmd, string text)
      : this(cmd)
    {
      commandText = text;
    }
    ...
    protected MyCatSqlConnection Connection => command.Connection;

    protected MySqlParameterCollection Parameters => command.Parameters;

    #endregion

    public virtual void Close(MyCatSqlDataReader reader)
    {
    }
    ...
    public virtual bool ExecuteNext()
    {
      ...
      MyCatSqlPacket packet = _buffers[0];
    ...
    protected virtual void BindParameters()
    {
      ...
      while (true)
      {
        ...
        {
          MyCatSqlCommand batchedCmd = command.Batch[index++];
          MyCatSqlPacket packet = (MyCatSqlPacket)_buffers[_buffers.Count - 1];
    ...
    private void InternalBindParameters(string sql, MySqlParameterCollection parameters, MyCatSqlPacket packet)
    {
      bool sqlServerMode = command.Connection.Settings.SqlServerMode;

      if (packet == null)
      {
        packet = new MyCatSqlPacket(Driver.Encoding) {Version = Driver.Version};
        packet.WriteByte(0);
      }
    ...
    private bool SerializeParameter(MySqlParameterCollection parameters,
                                    MyCatSqlPacket packet, string parmName, int parameterIndex)
    {
