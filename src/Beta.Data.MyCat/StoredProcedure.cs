...
namespace Beta.Data.MyCatSqlClient
{
  ...
  internal class StoredProcedure : PreparableStatement
  {
    ...
    public StoredProcedure(MyCatSqlCommand cmd, string text)
      : base(cmd, text)
    {
    }
    ...
    private string SetUserVariables(MySqlParameterCollection parms, bool preparing)
    {
      ...
      foreach (MySqlParameter p in parms)
      {
        ...
        {
          MyCatSqlCommand cmd = new MyCatSqlCommand(sql, command.Connection);
    ...
    internal void ProcessOutputParameters(MyCatSqlDataReader reader)
    {
    ...
    private void AdjustOutputTypes(MyCatSqlDataReader reader)
    {
    ...
    public override void Close(MyCatSqlDataReader reader)
    {
      base.Close(reader);
      if (String.IsNullOrEmpty(_outSelect)) return;
      if ((reader.CommandBehavior & CommandBehavior.SchemaOnly) != 0) return;

      MyCatSqlCommand cmd = new MyCatSqlCommand(_outSelect, command.Connection);
      using (MyCatSqlDataReader rdr = cmd.ExecuteReader(reader.CommandBehavior))
      {
