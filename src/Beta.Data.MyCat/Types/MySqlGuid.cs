...
namespace Beta.Data.Types
{
  ...
  internal struct MySqlGuid : IMySqlValue
  {
    ...
    void IMySqlValue.WriteValue(MyCatSqlPacket packet, bool binary, object val, int length)
    {
      ...
      else
      {
        ...
        if (binary)
          ...
          packet.WriteStringNoNull("'" + MyCatSqlHelper.EscapeString(guid.ToString("D")) + "'");
      }
    }

    private void WriteOldGuid(MyCatSqlPacket packet, Guid guid, bool binary)
    {
    ...
    private static void EscapeByteArray(byte[] bytes, int length, MyCatSqlPacket packet)
    {
    ...
    private MySqlGuid ReadOldGuid(MyCatSqlPacket packet, long length)
    {
      if (length == -1)
    ...
    IMySqlValue IMySqlValue.ReadValue(MyCatSqlPacket packet, long length, bool nullVal)
    {
    ...
    void IMySqlValue.SkipValue(MyCatSqlPacket packet)
    {
