...
namespace Beta.Data.Common
{
  ...
  internal class Ssl
  {
    ...
    /// </summary>
    private MyCatSqlConnectionStringBuilder _settings;
    ...
    public Ssl(MyCatSqlConnectionStringBuilder settings)
    {
    ...
    {
      this._settings = new MyCatSqlConnectionStringBuilder()
      {
        Server = server,
        SslMode = sslMode,
    ...
    /// <returns>A <see cref="MyCatSqlStream"/> instance ready to initiate an SSL connection.</returns>
    public MyCatSqlStream StartSSL(ref Stream baseStream, Encoding encoding, string connectionString)
    {
      ...
      baseStream = ss;
      MyCatSqlStream stream = new MyCatSqlStream(ss, encoding, false);
