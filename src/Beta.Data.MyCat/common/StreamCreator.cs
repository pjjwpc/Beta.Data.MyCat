...
using System.Net;
using System.Linq;
using Beta.Data.MyCatSqlClient.Common;
...
namespace Beta.Data.Common
{
  ...
  internal class StreamCreator
  {
    ...
    {
      MyCatSqlConnectionStringBuilder settings = new MyCatSqlConnectionStringBuilder
      {
        Server = server,
        Port = port,
        PipeName = pipename,
    ...
    public static Stream GetStream(MyCatSqlConnectionStringBuilder settings)
    {
      switch (settings.ConnectionProtocol)
      {
        case MyCatSqlConnectionProtocol.Tcp: return GetTcpStream(settings);
        case MyCatSqlConnectionProtocol.UnixSocket: return GetUnixSocketStream(settings);
        case MyCatSqlConnectionProtocol.SharedMemory: return GetSharedMemoryStream(settings);
        case MyCatSqlConnectionProtocol.NamedPipe: return GetNamedPipeStream(settings);
      }
      throw new InvalidOperationException(Resources.UnknownConnectionProtocol);
    }

    private static Stream GetTcpStream(MyCatSqlConnectionStringBuilder settings)
    {
    ...
    internal static Stream GetUnixSocketStream(MyCatSqlConnectionStringBuilder settings)
    {
      try
      {
    ...
    private static Stream GetSharedMemoryStream(MyCatSqlConnectionStringBuilder settings)
    {
    ...
    private static Stream GetNamedPipeStream(MyCatSqlConnectionStringBuilder settings)
    {
